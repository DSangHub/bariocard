...
export default function App(){
  const [memberId,setMemberId]=useState(localStorage.getItem('tbio_id')||'');
  ...
  const join = async (e)=>{
    ...
    if(data.memberId){localStorage.setItem('tbio_id',data.memberId); setMemberId(data.memberId);}
  };
  return (
    <div style={{background:'#0f0f0f',color:'#fff',minHeight:'100vh',padding:20,fontFamily:'sans-serif'}}>
      <h1 style={{color:'#f5b316'}}>The Bar Is Open - TBIO Voluntary 21+ Membership</h1>
      ...
      {!memberId ? (
        <form onSubmit={join} style={{display:'grid',gap:8,maxWidth:400}}>
          ...
          <button style={{background:'#f5b316',color:'#000',padding:10}}>Join Free - Get TBIO ID</button>
        </form>
      ):(
        <div>
